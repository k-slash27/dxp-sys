...
import { X, MapPin, Camera, Trash2, Loader2 } from 'lucide-react';
...
export default function JournalForm({
  ...
  return (
    <>
    {/* ------------------------------------------------------------------ */}
    {/* ファイル input: label[htmlFor] で開くため JS 不要、位置ピック後も確実に動作 */}
    {/* ------------------------------------------------------------------ */}
    <input
      ...
      onChange={e => {
        ...
        e.target.value = '';  // FileList クリア（files は既にスナップショット済）
        if (files.length > 0) setNewFiles(prev => [...prev, ...files]);
      }}
    />

    {/* 場所選択中: フローティングバナー（モーダルは display:none で維持）  */}
    {pickingLocation && (
      <div style={{
        position: 'fixed',
        bottom: '40px',
    ...
    {/* モーダルは常にマウントしたまま — pickingLocation 中は非表示にするだけ */}
    <div style={{...overlay, display: pickingLocation ? 'none' : undefined}}
      ...
      <div style={modal}>
        ...
        <form onSubmit={handleSubmit} style={{ padding: '20px 24px' }}>
          ...
          <div style={field}>
            ...
            {/* label[htmlFor] でファイル選択を開く — JS の .click() より確実 */}
            <label
          ...
          <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '10px', paddingTop: '8px' }}>
            <button type="button" onClick={onCancel} style={btnSecondary} disabled={saving}>
              キャンセル
            </button>
            <button type="submit" style={btnPrimary} disabled={saving}>
              {saving ? <><Loader2 size={14} /> 保存中…</> : '保存'}
            </button>
          </div>
        </form>
      </div>
