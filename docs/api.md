...
operator（Next.js）は register-service への直接アクセスをプロキシします。

### POST /api/upload

register-service の `POST /webhook/upload-file` へのストリームプロキシ。  
`bodyParser` を無効化し、リクエストボディをそのまま転送することで大容量ファイルに対応しています。

**動作**: `multipart/form-data` リクエストをそのまま register-service に転送

**フォームフィールド**: register-service と同一（`file`, `workspace`, `date`）

**内部転送先**: `http://register-service:8000/webhook/upload-file`

---

### GET /api/progress/[taskId]

register-service の `GET /webhook/progress/{task_id}` への SSE プロキシ。

**内部転送先**: `http://register-service:8000/webhook/progress/{taskId}`

---

### GET /api/areas

register-service の `GET /webhook/areas` へのプロキシ。

**内部転送先**: `http://register-service:8000/webhook/areas`
